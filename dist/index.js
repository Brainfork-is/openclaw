...
function normalizeSearchResults(payload) {
    ...
    if (Array.isArray(record.results)) {
        return record.results.filter((entry) => entry && typeof entry === "object");
    }
    if (Array.isArray(record.items)) {
        return record.items.filter((entry) => entry && typeof entry === "object");
    }
    if (Array.isArray(record.documents)) {
        return record.documents.filter((entry) => entry && typeof entry === "object");
    }
    if (Array.isArray(record.matches)) {
        return record.matches.filter((entry) => entry && typeof entry === "object");
    }
    return [];
}
...
async function searchBrainfork(client, query, limit) {
...
function toSimilarityScore(value) {
    if (typeof value === "number" && Number.isFinite(value)) {
        return value;
    }
    return undefined;
}
function normalizeRagResults(payload) {
    const results = normalizeSearchResults(payload);
    return results.map((item) => ({
        ...item,
        score: toSimilarityScore(item.score) ??
            toSimilarityScore(item.metadata?.score) ??
            toSimilarityScore(item.metadata?.similarity) ??
...
function extractRemoteDocumentMetadata(doc, payload) {
    ...
    return {
        remoteId: firstString(payload, "id", "documentId", "resourceId") ??
            firstString(nestedDocument, "id", "documentId", "resourceId"),
        remoteUrl: firstString(payload, "url", "documentUrl") ?? firstString(nestedDocument, "url", "documentUrl"),
    ...
    for (const action of plan) {
        try {
            ...
            if (action.type === "cleanup") {
                ...
                state = applyRemovedResult(state, action.entry, action.mode === "archive" ? "archived" : "deleted");
...
const brainforkPlugin = {
    ...
    register(api) {
        ...
        api.registerTool({
            ...
            async execute(_toolCallId, params) {
                const response = await client.callToolParsed("get_decisions", params);
                return jsonResult(response.parsedText ?? response.raw);
            },
        }, { name: "brainfork_get_decisions" });
        api.registerTool({
        ...
        api.registerCli(({ program, workspaceDir }) => {
            ...
            brainfork
                ...
                printStatusLine("tools", tools.length > 0
                    ? tools
                        .map((tool) => tool.name)
                        .filter(Boolean)
                        .join(", ")
            ...
            if (config.autoIndex && workspaceDir) {
                try {
                    ...
                    if (total > 0 || summary.archived > 0 || summary.deleted > 0) {
                        api.logger.info(`[brainfork-openclaw] sync ${path.basename(workspaceDir)} indexed=${summary.indexed} changed=${summary.changed} unchanged=${summary.unchanged} archived=${summary.archived} deleted=${summary.deleted}`);
                    }
                }
                catch (error) {
