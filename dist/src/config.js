...
function readInteger(value, key, options) {
    const raw = value[key];
...
function readNumber(value, key, options) {
    const raw = value[key];
    const candidate = typeof raw === "number" ? raw : options.fallback;
...
function readDeleteMode(value) {
    const raw = value.deleteMode;
    if (raw === undefined) {
        return "archive";
    }
    if (typeof raw !== "string" || !DELETE_MODES.includes(raw)) {
