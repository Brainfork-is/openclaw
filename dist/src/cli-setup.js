...
function buildAuthorizeUrl(baseUrl, redirectUri, state, challenge) {
...
export function detectEndpointFromAccessToken(accessToken) {
...
function createPromptApi() {
    ...
    async function askSecret(message) {
        const mutableOutput = process.stdout;
        ...
        mutableOutput.write = ((chunk, encoding, cb) => {
            ...
            return originalWrite(chunk, safeEncoding, cb);
        });
        try {
...
async function runManualSetup(prompts, configPath) {
...
export function registerBrainforkSetupCommand(options) {
    options.brainfork
        .command("setup")
        ...
        if (cliOpts.baseUrl || cliOpts.apiKey || cliOpts.endpoint) {
            throw new Error("Non-interactive setup requires all three flags: --base-url, --api-key, and --endpoint");
        }
